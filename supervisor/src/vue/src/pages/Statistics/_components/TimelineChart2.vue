    ...
    const option: echarts.EChartsOption = {
      tooltip: {
        ...
        formatter: (params) => {
          ...
          if (!itemData.value[2]) return '';

          const name = itemData.name;
          const startTime = new Date(itemData.value[1]);
          const endTime = new Date(itemData.value[2]);
          const duration = (endTime.getTime() - startTime.getTime()).toFixed(0);

          return `<b>Węzeł:</b> ${name}<br/>` + `<b>Czas trwania:</b> ${duration} ms`;
        },
      },
      grid: {
        containLabel: true,
...
onBeforeMount(async () => {
  ...
  seriesData.value = messages.map((msg) => createItem(msg));
});
...
function createItem(msg: NodeMessage): ChartItem {
  const nodeName = NodeUtilities.formatId(msg.senderId);
  return {
    id: msg.eventId,
    name: nodeName,
    value: [nodeName, msg.startDate.getTime(), msg.endDate?.getTime()],
  };
}

onLoRaEvent({
  onMessage(msg: NodeMessage) {
    if (!msg.endDate) return;
